...
def _build_checklist_workbook(*, lot_number, generated_on, generated_by, grade, groups,
                              ...
                              labour_cost, total_estimation, severity_label) -> bytes:
    ...
    ws["A3"].font = bold
    ws["A4"] = f"Cosmetic Damage Scope: {severity_label}"
    ...
    ws.cell(row=r, column=4, value="Total Labour Cost").font = bold
    lc = ws.cell(row=r, column=5, value=float(labour_cost))
    ...
    r += 1
    ws.cell(row=r, column=4, value="Total Estimate").font = bold
    te = ws.cell(row=r, column=5, value=float(total_estimation))
...
async def generate_checklist_estimate(
    ...
    payload = {
      severity: ["minor","major"],   # which severities are summed into "sev" fields
      prices: {"<model>||<group>|<field>": unit_price},
      grade: grade_value,
      labour_cost, notes
    }

    Counts are always re-derived here from IQC/Device data — the browser only
    supplies the severity scope, the prices, and which grade to label the
    ...
    severity = {s for s in (data.get("severity") or []) if s in ("minor", "major")}
    if not severity:
        severity = {"minor", "major"}
    severity_label = "Minor + Major" if severity == {"minor", "major"} else \
        ("Minor only" if severity == {"minor"} else "Major only")
    ...
    qtys = checklist_svc.quantities_for(matrix, severity)
            ...
            for m_idx, row in enumerate(matrix):
                ...
                if count or price:
                    checklist_lines.append(PartEstimateChecklistLine(
                        ...
                        severity_scope=severity_label, count=count,
    ...
    total_products = (await device_counts_by_lot(db)).get(lid, 0)
    total_qty = sum(cell["count"] for cell in cells.values())
    stamp = app_now()
    os.makedirs(UPLOAD_DIR, exist_ok=True)
    safe_lot = "".join(ch for ch in (lot.lot_number or "lot") if ch.isalnum() or ch in "-_")
    ...
    xlsx = _build_checklist_workbook(
        ...
        labour_cost=labour, total_estimation=total_estimation, severity_label=severity_label,
    )
    safe_grade = "".join(ch for ch in grade_label if ch.isalnum() or ch in "-_") or "grade"
    stored = f"{uuid.uuid4().hex}.xlsx"
    ...
    await audit(db, user=current_user, action="PART_ESTIMATE_GENERATED",
                ...
                new_value={"lot": lot.lot_number, "mode": "checklist", "grade": grade,
                           "severity": severity_label, "total_qty": total_qty,
                           "total_estimation": str(total_estimation)},
                request=request)
    await db.commit()
